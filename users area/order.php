...
while($row_price=mysqli_fetch_array($result_cart_price)){
    ...
    $run_price=mysqli_query($connect,$select_products);
    while($row_product_price=mysqli_fetch_array($run_price)){
        $product_price=array($row_product_price['product_price']);
        $sum_product_price=array_sum($product_price);
        $total_price+=$sum_product_price;
    }
}
...
if($result_query){
    echo "<script>alert('Congratulations!! Order Placed Successfully')</script>";
...
//delete items from cart after orders placed
